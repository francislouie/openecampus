...
class sms_dmc_format2_parser(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(sms_dmc_format2_parser, self).__init__(cr, uid, name, context = context)
    ...
    def get_students_dmc(self,form):
        ...
        for row in student_rows:
            ...
            final_dict = {'image':'','result':'','total_students':'','position':'','grade':'','remarks':'','student_class_status':'','cadidate_no':'','student_name':'','father_name':'','section':'','gender':'','total_obtained_marks':'','total_obtained_percentage':'','class_total_marks':''}
            
            sql = """SELECT sms_student.registration_no,
                     sms_student.name, 
                     sms_student.father_name, 
                     sms_student.gender,
                     current_class,
                     sms_student.image 
                    from sms_student
...
report_sxw.report_sxw('report.sms_dmc_format2_name', 'sms.student', 'addons/sms/report/sms_dmc_formate2.rml',parser = sms_dmc_format2_parser, header='External')
