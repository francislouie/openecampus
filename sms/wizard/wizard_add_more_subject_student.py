...
class add_subject_student(osv.osv_memory):
    ...
    def _get_student(self, cr, uid, ids):
        ...
        ctx.update({
        'class_id': stdobj.current_class.id,
        'student_id': std_id,
        'acad_cal_student': self.pool.get('sms.academiccalendar.student').search(self,cr,uid,[('student','=',std_id),('name','=',stdobj.current_class.id),('state','=','Current')])[0],
    ...
    def _get_active_class(self, cr, uid, context=context):
         
        if context['class_id']:
            return context['class_id']
        else:
            return []
    ...
    _columns = {
              ...
              "name": fields.many2one('sms.academiccalendar', 'Class', domain="[('admission_closed','=',False),('state','!=','Complete'),('subjects_loaded','=',True),('fee_defined','=',True)]", required=True, help="1: Only Draft & Active Classes are displayed here.\n2: Class Fee must be set before getting admission in that class. "),
    ...
    def register_student(self, cr, uid, ids, data):
        ...
        for f in self.browse(cr, uid, ids):
            ...
            if std_cal_id:
                ...
                print "this is registration no",admn_no
...
#               first insert all fees(search for fee with subtype at_admission)
