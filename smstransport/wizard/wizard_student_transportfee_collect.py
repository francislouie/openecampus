...
class class_student_transportfee_collect(osv.osv_memory):
    ...
    _name = "class.student_transportfee_collect"
    _description = "Student's Fee Collection"
    _columns = {
              'student_id': fields.many2one('sms.student', 'Student', domain="[('state','=','Admitted')]", help="Student"),
              'challan_id': fields.many2one('sms.transportfee.challan.book', 'Challan', domain="[('state','=','fee_calculated'),('student_id','=',student_id)]", help="Challan", required=True),
               }
    _defaults = {'student_id':_get_student}

    def action_pay_student_transportfee(self, cr, uid, ids, context):
        ctx = {}
        ...
        result = {
                ...
                'name': 'Collect Fees',
                'res_model': 'sms.transportfee.challan.book',
                'view_type': 'form',
                'view_mode': 'form',
                'view_id': False,
                'nodestroy': True,
...
class_student_transportfee_collect()
